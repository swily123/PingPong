...
public class Predictor : MonoBehaviour
{
    ...
    public Vector3 Predict(bool isPlayerSide, out float time)
    {
        ...
        _simulatingBody.transform.rotation = _ball.transform.rotation;
        _simulatingBody.velocity = _ball.GetComponent<Rigidbody>().velocity;
        
        for (int i = 0; i < _count; i++)
        {
            _simulationScene.Simulate(_step);
            time += _step;
            //var phantom = Instantiate(_phantomPrefab, _simulatingBody.position, Quaternion.identity);
